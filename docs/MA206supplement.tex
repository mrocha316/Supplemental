...
\begin{itemize}
\item
  \textbf{describe} - (define here)
\item
  \textbf{predict} - (define here)
\item
  \textbf{cause-and-effect} - (define here)
\end{itemize}

For example, let's say a medical researcher investigates a newly discovered variant of the coronavirus. Initially, researchers will probably focus on describe studies. \emph{What is the prevalence of the variant in the population?} \emph{What groups of individuals are most vulnerable to the variant?} Follow-on studies will have the p (continue example here)
